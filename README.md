...
```sh
cd ~
git clone --branch v1.7.6 --depth 1 https://github.com/bokiko/bloxos.git
cd ~/bloxos/docker
cp .env.example .env
```

This downloads release **v1.7.6**, enters its Docker folder, and makes your own
...
Confirm your address and `BLOXOS_VERSION=1.7.6` appear without a `#` before them.
...
set it to `1.7.6`, then save with **Ctrl+O**, **Enter**, **Ctrl+X**. Keep your
...
The v1.7.6 packages carry agent release **9** for both native and Docker
installations, unchanged from v1.7.5. This release updates the hub and
dashboard; it does not introduce a new agent build. Updating a server does
...
[Release notes](https://github.com/bokiko/bloxos/releases/tag/v1.7.6) ·
